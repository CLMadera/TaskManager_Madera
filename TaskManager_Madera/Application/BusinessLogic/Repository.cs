...
namespace TaskManager.BusinessLogic
{
    public class Repository : IRepository
    {
        ...
        public async Task<IEnumerable<SeriLog>> GetAllAsync()
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                return await connection.QueryAsync<SeriLog>(sql);
            }
        }
        public async Task<SeriLog> GetByIdAsync(int id)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                return (await connection.QueryAsync<SeriLog>(sql, new { Id = id })).FirstOrDefault();
            }
        }
        public async Task<IEnumerable<LogCountByDayDto>> GetLogCountByDayAsync()
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var sql = @"
    ...
    ORDER BY Day";
                var results = await connection.QueryAsync<LogCountByDayDto>(sql);
                return results;
            }
        }
        public async Task<IEnumerable<SeriLog>> GetByLogLevelAsync(string logLevel)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var sql = @"
        ...
        FROM SeriLogs
        WHERE Level = @Level";
                var results = await connection.QueryAsync<SeriLog>(sql, new { Level = logLevel });
                return results;
            }
        }
        public async Task<IEnumerable<SeriLog>> GetByMessageAsync(string searchTerm)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var sql = "SELECT * FROM SeriLogs WHERE Message LIKE @SearchTerm";
                return await connection.QueryAsync<SeriLog>(sql, new { SearchTerm = $"%{searchTerm}%" });
            }
        }
    }
}